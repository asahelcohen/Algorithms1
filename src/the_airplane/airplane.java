...
public class airplane {
    ...
    public static class BestPath {
        ...
        private void buildMatrix() {
            ...
            numOfPaths = mat[column - 1][row - 1].numOfPaths;
            cheapestPrice = mat[column - 1][row - 1].price;
        }
        ...
        private void getAllPathsRec(String temp, int i, int j, Vector<String> ans) {
            if (i == 0 && j == 0) { // if we reached 0,0 then stop function
                ans.add(temp);
                return;
            } else if (i > 0 && j == 0) {// if we reached j wall --> move upwards only
                getAllPathsRec("1" + temp, i - 1, 0, ans);
            } else if (i == 0 && j > 0) {// if we reached i wall --> move left only
                getAllPathsRec("0" + temp, 0, j - 1, ans);
            } else { // choose best option (going backwards)
                int a = mat[i - 1][j].price + mat[i - 1][j].y;
                int b = mat[i][j - 1].price + mat[i][j - 1].x;
                if (a < b) {
                ...
                } else { // if a==b split the recursion into two
                    getAllPathsRec("1" + temp, i - 1, j, ans);
                    getAllPathsRec("0" + temp, i, j - 1, ans);
                }
            }
        ...
        public boolean isOnBestPath(int i, int j) {
            ...
            return (mat[i][j].price + mat[i][j].entryFromTheEnd) == cheapestPrice;
        }
        ...
        private void buildMatrixFromTheEnd() {
            ...
            for (int i = n - 1; i >= 0; i--) {
                for (int j = m - 1; j >= 0; j--) {
                    ...
                    mat[i][j].entryFromTheEnd = Math.min(x, y);
                }
            }
        }
...
//            System.out.println(row_length+"----->"+columns_length+"----->"+temp.length+"------>"+temp[0].length);
            ...
            // set the prices for the first row
            for (int i = 1; i < row_length; i++) {
                temp[i][0].price = mat[i - 1 + y1][x1].y + temp[i - 1][0].price;
            }
